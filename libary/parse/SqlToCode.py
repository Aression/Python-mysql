...
bye = """\033[1;31mProgram terminating....\033[0m"""
                ...
                if "from" in sql:
                    ...
                    if "where" not in sql:
                        ...
                        try:
                            ...
                            if len(wheres) != len(relations):
                                print("\033[1;31mERROR : where语句解析错误0\033[0m")
                            db.select_data_from_table_with_where(
                                table, columns, wheres, relations, limit
                            )
        ...
        #  insert into table (id,name) values ('1','2') , ('1','2');
            ...
            if sql_arr[1] == "into" and "values" in sql:
                try:
                    ...
                    columns = (
                        sql.split(")")[0].split("(")[1].replace(" ", "").split(",")
                    )
                        ...
                        for y in range(0, len(values[x])):
                            if "'" in values[x][y] or '"' in values[x][y]:
                                ...
                                try:
                                    ...
                                    print(
                                        "\033[1;31mERROR : 类型转化错误 检查字符串是否被引号包裹\033[0m"
                                    )
        ...
        # 转化为 db.update_from_table("table",{"a":1},{"id":["=",2]})
            ...
            if "where" in sql:
                ...
                for _set in tmp_sets:
                    sets.update(
                        {_set.split("=")[0].strip(): _set.split("=")[1].strip()}
                    )
                tmp_wheres = re.findall(
                    "[a-zA-Z0-9]+[\ ]*[><=!]+[\ ]*[a-zA-Z0-9'\"]+",
                    sql.split("where")[1],
